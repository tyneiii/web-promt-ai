...
function getActionButtonsHTML() {
    ...
    if (promptStatus === 'waiting') {
        buttons += `
            <button class="action-btn run-btn" onclick="if(confirm('Duyệt bài đăng này?')) handleAction('approve', currentPromptId, '')">
                Duyệt bài đăng
            </button>
            <button class="action-btn back-btn" onclick="if(confirm('Từ chối bài đăng này?')) {
        ...
        buttons += `
            <button class="action-btn run-btn" onclick="if(confirm('Xóa báo cáo và khôi phục bài?')) handleAction('unreport', currentPromptId)">
                Bỏ báo cáo (Duyệt lại)
            </button>
            <button class="action-btn back-btn" style="background:#e74c3c;" onclick="if(confirm('XÓA VĨNH VIỄN bài đăng này?')) handleAction('delete', currentPromptId)">
                Xóa bài đăng
            </button>
        `;
    } else {
